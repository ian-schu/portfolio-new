...
// Start typey
typeControl(likeSpan, blurbArray);

// Drop patterns
const dropAllPatterns = () => {
	if (!deviceIsMobile) {
		const gliders = [[bHeight * 0.1, bWidth * 0.1], [bHeight * 0.25, bWidth * 0.4]];
		const hives = [[bHeight * 0.65, bWidth * 0.1], [bHeight * 0.9, bWidth * 0.8]];
		const blocks = [[bHeight * 0.27, bWidth * 0.7], [bHeight * 0.8, bWidth * 0.1]];
		const spaceships = [[bHeight * 0.4, bWidth * 0.75], [bHeight * 0.68, bWidth * 0.55]];
